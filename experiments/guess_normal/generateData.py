...
NODE_SELECTION_H=["MaxMax"]

############################################
## Constants for events
############################################
EVENTS_LENGTH=100000
# ADDITION=[0.8,0.9,1]
node_sizes=[8,16]
...
if __name__=='__main__':
    ...
    users=UserTiming(services,num_users=100,num_domains=NUM_DOMAINS,dir=main_dir)
    print("users are generated")
    EventGenerator(EVENTS_LENGTH,weight=0.6,Users=users,Services=services,dir=events_dir)
    print("events are generated")
    
    
    
    Users=pd.read_csv(f'{main_dir}/users.csv')
    Users["Services"] = Users["Services"].apply(ast.literal_eval)
    Users["Domains"] = Users["Domains"].apply(ast.literal_eval)
    Services=pd.read_csv(f'{main_dir}/services/services0.csv')
    ...
    #             domainNodesUpperBound([opt0,opt1],main_dir+f'domainNodes{s}/{opt0}/{opt1}/Reserved/',Users=Users,Services=Services,num_cores=s,num_domains=NUM_DOMAINS,num_init_nodes=[1]*10,num_cores_init=s)
    ...
    #         domainNodesUpperBound([opt0,opt1],main_dir+f'domainNodesVariable/{opt0}/{opt1}/Reserved/',Users=Users,Services=Services,num_cores=NUM_CORES_PER_SCALED_NODE,num_domains=NUM_DOMAINS,num_init_nodes=[1]*10,num_cores_init=NUM_CORES_PER_INIT_NODE)
