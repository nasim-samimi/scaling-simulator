...
plots='experiments/plots_with_cores/'
...
# Actual average workload (total cores system-wide) computed from event files
# These are the real measured average demands across all domains + cloud
    ...
    for node_size in node_sizes:
        for metric in ['cost','qos','qosPerCost']:
            ...
            for l in leg:
                ...
                plt.plot(avgs['addition'].map(WORKLOAD_CORES_SYSTEMWIDE), avgs['averages'], marker='o')
            plt.grid(True)
            plt.xlabel('Average Core Demand')
        ...
        for p in partitionHeus:
            ...
            for l in top_heuristics:
                if 'baseline' in l:
                    ...
                    avgs = averages[averages['heuristics'] == l]
                    avgs = avgs.sort_values(by='addition')
                    plt.plot(avgs['addition'].map(WORKLOAD_CORES_SYSTEMWIDE), avgs['averages'], marker=marker,linestyle=linestyle, label=l,linewidth=linewidth,markersize=8,color='black')
                else:
                    marker='x'
                    linestyle='-'
                    avgs = averages[averages['heuristics'] == l]
                    avgs = avgs.sort_values(by='addition')
                    plt.plot(avgs['addition'].map(WORKLOAD_CORES_SYSTEMWIDE), avgs['averages'], marker=marker,linestyle=linestyle, label=l,linewidth=linewidth,markersize=8)
            plt.grid(True)
            plt.xlabel('Average Core Demand')
            # plt.xticks(range(len(ADDITIONS)), ADDITIONS)
            plt.ylabel(metric)
...
def robustness_compare_node_core_selection(dir1='improved/allOpts',dir2='baseline',metric='cost',flags='allOpts',nodesize=8):
    ...
    for m in max_size:
        ...
        plt.grid(True)
        plt.xlabel('Average Core Demand',fontsize=fontsizetxt)
            ...
            for c in cost:   
                ...
                for l in top_heuristics:
                    ...
                    if 'baseline' in l:
                        marker='x'
                        linestyle='-'
                        plt.plot([WORKLOAD_CORES_SYSTEMWIDE[a] for a in ADDITIONS], avgs['averages'], marker=marker,linestyle=linestyle, label=l,linewidth=linewidth,markersize=8)
                    else:
                        marker='o'
                        linestyle='--'
                        plt.plot([WORKLOAD_CORES_SYSTEMWIDE[a] for a in ADDITIONS], avgs['averages'], marker=marker,linestyle=linestyle, label=l,linewidth=linewidth,markersize=8)
...
def robustness_compare_nodesize(dir1='improved/allOpts',dir2='baseline',metric='cost',flags='allOpts'):
    ...
    for m in max_size:    
        ...
        print(f'{savingDir}robustness_{metric}_all_nodesizes.svg')
        plt.close()
...
def robustness_compare_nodesize_none(dir1='improved/allOpts',dir2='baseline',metric='cost',flags='allOpts'):
    ...
    for m in max_size:    
        ...
        print(f'{savingDir}robustness_{metric}_all_none_nodesizes.svg')
        plt.close()
            ...
            for p in partitionHeus:
                ...
                for l in top_heuristics:
                    if 'baseline' in l and 'e-' not in l:
                        ...
                        avgs = averages[averages['heuristics'] == l]
                        avgs = avgs.sort_values(by='addition')
                        plt.plot(avgs['addition'].map(WORKLOAD_CORES_SYSTEMWIDE), avgs['averages'], marker=marker,linestyle=linestyle, label=l,linewidth=linewidth,markersize=8,color='green')
                    else:
                        marker='o'
                        linestyle='--'
                        avgs = averages[averages['heuristics'] == l]
...
def robustness_max_scaling_size_3d_sheets(dir1='improved/allOpts', dir2='baseline', metric='cost', flags='allOpts', nodesize=8):
    ...
    for n in nodeHeus:
        for p in partitionHeus:
            ...
            ax.set_box_aspect([2, 1, 1])  # Balance axis lengths
            ...
            for l in top_heuristics:
                ...
                for m in max_size:
                    ...
                    avgs = avgs.sort_values(by='addition')
                    X_vals.extend(avgs['addition'].map(WORKLOAD_CORES_SYSTEMWIDE).values)
                ...
                if 'cost' in metric:
                    if l=='baseline':
                        ...
                        t=1000
                    ax.text(X_vals.max()-0.3, Y_vals.mean(), t , l, color='black', fontsize=9, fontweight='bold', bbox=dict(facecolor='white', alpha=0.7))
                if 'qos' in metric:
                    if l=='baseline':
                        # ax.plot_wireframe(grid_x, grid_y, grid_z, color='black', linewidth=1.5)  # Wireframe for baseline
                        t=250
                        ...
                        t=300
                    ax.text(X_vals.min(), Y_vals.min(), t , l, color='black', fontsize=9, fontweight='bold', bbox=dict(facecolor='white', alpha=0.7))
                if 'qosPerCost' in metric:
                    if l=='baseline':
                        ...
                        t=0.5
                    ax.text(X_vals.min(), Y_vals.max(), t , l, color='black', fontsize=9, fontweight='bold', bbox=dict(facecolor='white', alpha=0.7))
                # ax.text(X_vals.mean(), Y_vals.mean(), Z_vals.max(), l, color='black', fontsize=9, fontweight='bold')
                

            # Set 3D plot labels and grid
            ax.set_xlabel('Average Core Demand',labelpad=20,fontsize=12)
            ax.set_ylabel('Max Scaling Size',labelpad=5,fontsize=12)
            if 'cost' in metric:
                ax.set_zlabel('cost',labelpad=3,fontsize=12)
            elif 'qosPer' in metric:
                ax.set_zlabel('QoS per cost',labelpad=3,fontsize=12)
            else:
                ax.set_zlabel('QoS',labelpad=3,fontsize=12)
            ax.tick_params(axis='both', which='major', labelsize=10)
            # Set Y-axis ticks explicitly to avoid overlap
            ax.set_yticks([16, 32, 64, 128])
            # Rotate Y-axis labels like interval-based plots
...
def robustness_interval_length_3d_sheets(dir1='improved/allOpts', dir2='baseline', metric='cost', flags='allOpts', nodesize=8):
    ...
    for n in nodeHeus:
        for p in partitionHeus:
            ...
            ax.set_xlabel('Average Core Demand',labelpad=20,fontsize=12)
            ax.set_ylabel('Interval Length',labelpad=5,fontsize=12)
    ...
    for m in max_sizes:
        for n in nodeHeus:
            for p in partitionHeus:
                ...
                for l in top_heuristics:
                    ...
                    if 'cost' in metric:
                        if l=='baseline':
                            ...
                            t=360
                        ax.text(X_vals.max(), Y_vals.min(), t , l, color='black', fontsize=9, fontweight='bold', bbox=dict(facecolor='white', alpha=0.7))
                    if 'qos' in metric:
                        if l=='baseline':
                            # ax.plot_wireframe(grid_x, grid_y, grid_z, color='black', linewidth=1.5)  # Wireframe for baseline
                            t=170
                ...
                # Set 3D plot labels and grid
                ax.set_xlabel('Average Core Demand',labelpad=20,fontsize=12)
                ax.set_ylabel('Cloud Cost',labelpad=5,fontsize=12)
                ax.set_zlabel(metric,labelpad=3,fontsize=12)
                ax.tick_params(axis='both', which='major', labelsize=10)
                if 'cost' in metric:
                    ax.view_init(azim=150) 
                if 'qos' in metric and 'Per' not in metric:
                    ax.view_init(azim=120)  # Rotated left from original
                if 'qosPer' in metric:
                    ax.view_init(azim=120)  # Rotated left from original 
                # ax.set_title(f'3D Robustness Comparison for {metric}\nNode: {n}, Partition: {p}')
                ax.grid(True)
    ...
    for m in max_sizes:
        for n in nodeHeus:
            for p in partitionHeus:
                ...
                # Set 3D plot labels and grid
                ax.set_xlabel('Average Core Demand',labelpad=20,fontsize=12)
                ax.set_ylabel('QoS Ratio',labelpad=5,fontsize=12)
                ax.set_zlabel(metric,labelpad=3,fontsize=12)
                ax.tick_params(axis='both', which='major', labelsize=10)
                if 'cost' in metric:
                    ax.view_init(azim=150) 
            ...
            for p in partitionHeus:
                ...
                for l in top_heuristics:
                    if 'baseline' in l and 'e-' not in l:
                        ...
                        avgs = averages[averages['heuristics'] == l]
                        avgs = avgs.sort_values(by='addition')
                        plt.plot(avgs['addition'].map(WORKLOAD_CORES_SYSTEMWIDE), avgs['averages'], marker=marker,linestyle=linestyle, label=l,linewidth=linewidth,markersize=8,color='blue')
                    else:
                        marker='o'
                        linestyle='--'
                        avgs = averages[averages['heuristics'] == l]
                        avgs = avgs.sort_values(by='addition')
                        plt.plot(avgs['addition'].map(WORKLOAD_CORES_SYSTEMWIDE), avgs['averages'], marker=marker,linestyle=linestyle, label=l,linewidth=linewidth,markersize=8,color='red')
                plt.grid(True)
                plt.xlabel('Average Core Demand',fontsize=fontsizetxt)
                if 'cost' in metric:
                    plt.ylabel('cost',fontsize=fontsizetxt)
...
# Cloud Cost Comparison Functions
def robustness_compare_cloudcost(flags='allOpts'):
    dir1='allOpts'
    dir2='baseline'
    avg=[]
...
# Cloud Cost 3D Sheets
def robustness_cloud_cost_3d_sheets(dir1='improved/allOpts', dir2='baseline', metric='cost', flags='allOpts', nodesize=8):
    dirs = [main_dir + dir1, main_dir + dir2]
    cloud_cost = np.array([ 1,2,3,4,5])  # Different cloud costs
    nodeHeus=['mmRB']
    nodesize=8
    max_sizes=[16,32,64,128]
    dirs=main_dir+"cloud_cost/"
        ...
        for n in nodeHeus:
            for p in partitionHeus:
                ...
                for c in cloud_cost:
                    ...
                    # Store all values including cloud_cost for 3D plotting
                    averages['cloud_cost'] = c
                    all_averages = pd.concat([all_averages, averages], ignore_index=True)
                ...
                for l in top_heuristics:
                    ...
                    for c in cloud_cost:
                        ...
                        avgs = avgs.sort_values(by='addition')
                        # Map additions to core demand for x-axis
                        X_vals.extend(avgs['addition'].map(WORKLOAD_CORES_SYSTEMWIDE).values)
                        Y_vals.extend([c] * len(avgs))  # Y-axis is cloud cost
                        Z_vals.extend(avgs['averages'].values)
...
def robustness_qos_3d_sheets(dir1='improved/allOpts', dir2='baseline', metric='cost', flags='allOpts', nodesize=8):
    dirs = [main_dir + dir1, main_dir + dir2]
    qos = np.array([ 0.2,0.5,0.8])  # Different QoS ratios
    nodeHeus=['mmRB']
    nodesize=8
    max_sizes=[16,32,64,128]
    dirs=main_dir+"QoS/"
    for m in max_sizes:
        for n in nodeHeus:
            for p in partitionHeus:
                all_averages = pd.DataFrame(columns=['averages', 'heuristics', 'addition', 'QoS'])
                ...
                for c in qos:
                    ...
                    # Store all values including QoS for 3D plotting
                    averages['QoS'] = c
                    all_averages = pd.concat([all_averages, averages], ignore_index=True)
                ...
                for l in top_heuristics:
                    ...
                    for c in qos:
                        ...
                        # Map additions to core demand for x-axis
                        X_vals.extend(avgs['addition'].map(WORKLOAD_CORES_SYSTEMWIDE).values)
                        Y_vals.extend([c] * len(avgs))  # Y-axis is QoS ratio
                        Z_vals.extend(avgs['averages'].values)
