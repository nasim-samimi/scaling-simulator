...
sCoresRange=range(2,8)#range(2,11,2) # max number of cores should be matched with the number of cores in the system
...
def ServiceGenerator(numServices,importance,sBandwidth,sCores,number,dir): # it returns random values for the services for now
    seed=0
    ...
    if not os.path.exists(f'{dir}services/'):
        os.mkdir(f'{dir}services/')
    Services.to_csv(f'{dir}services/services{number}.csv', index=False)
    return Services
