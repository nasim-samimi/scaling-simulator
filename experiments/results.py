...
nodeHeu='MinMin'
partitionHeu='worstfit'

# nodeHeu='MaxMax'
# partitionHeu='bestfit'
...
def runtimes(dir1='heuristics/',dir2='baseline/'):
    ...
    for dir in dirs:
        for files in os.listdir(dir):
            if nodeHeu in files and partitionHeu in files:
    ...
    plt.legend(leg)
    plt.savefig(f'{main_dir}runs_{nodeHeu}_{partitionHeu}.png')
    plt.close()
    return
...
def qosPerCost():
    ...
    plt.legend(leg)
    # plt.show()
    plt.savefig(f'{main_dir}QpC_{nodeHeu}_{partitionHeu}.png')
    plt.close()
    return
